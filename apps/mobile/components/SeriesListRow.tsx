...
import type { CollectionVM } from "@/lib/catalog";
import { font } from "@/lib/fonts";
import { useI18n } from "@/lib/i18n";
import { useTheme } from "@/lib/theme";

/** Collection row for Library's Occasions/Series/Topics segments: cover + kind + meta. */
export function SeriesListRow({ series, onPress }: { series: CollectionVM; onPress?: () => void }) {
  const t = useTheme();
  const { t: msgs } = useI18n();
  return (
    <Pressable style={[styles.row, { backgroundColor: t.c.surface, borderColor: t.c.border }]} onPress={onPress}>
      <GradientCover gradient={series.cover.gradient} style={styles.cover}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        ...
        <Text style={[styles.meta, { color: t.c.textMuted }]}>
          {series.count} {msgs.library.parts} · {series.language.toUpperCase()}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  kind: { fontFamily: font.sans.extrabold, fontSize: 9.5, letterSpacing: 0.6, textTransform: "capitalize" },
